...
- **Branding**: renamed addon to `tech-skull-presence`, new logo (`tech_skull_clean.svg`), panel icon `mdi:skull-scan-outline`. Persistent data dir moved to `/config/tech-skull-presence`.
...
- Firmware update flow still references upstream EP firmware indexes; HP devices show "no firmware available" until we host an HP firmware index.
...
## Upstream history (Zone Configurator)
